"use client"
/**
 * V58.5 — /mon-logement/historique
 *
 * Liste les baux passés du locataire (miroir de /proprietaire/baux/historique).
 * Lecture seule. Source : /api/baux/historique?as=locataire.
 */
...
const FIN_MOTIF_LABEL: Record<string, string> = {
  preavis_locataire: "Préavis donné par vous",
...
export default function HistoriqueLocataire() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/baux/historique?as=locataire", { cache: "no-store" })
    ...
    <main style={{ minHeight: "100vh", background: "#F7F4EF", fontFamily: "'DM Sans', sans-serif", padding: "32px 16px" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <Link href="/mon-logement" style={{ fontSize: 12, color: "#8a8477", textDecoration: "none" }}>← Retour</Link>
        </div>
        ...
        <p style={{ fontSize: 14, color: "#666", margin: "0 0 24px", lineHeight: 1.5, maxWidth: 560 }}>
          Tous les baux que vous avez signés sur KeyMatch, du plus récent au
          plus ancien. Le bail signé, l&apos;EDL d&apos;entrée et de sortie
          restent accessibles 3 ans (loi ALUR).
        </p>
        ...
        {!loading && !error && baux.length === 0 && (
          <div style={{ background: "white", border: "1px solid #EAE6DF", borderRadius: 20, padding: 32, textAlign: "center" }}>
            <p style={{ fontFamily: "'Fraunces', Georgia, serif", fontStyle: "italic", fontSize: 18, color: "#111", margin: "0 0 8px" }}>
              Aucun ancien logement
            </p>
            <p style={{ fontSize: 13, color: "#8a8477", margin: 0 }}>
              Quand vous aurez quitté un logement, son bail apparaîtra ici.
            </p>
            <Link href="/annonces" style={{ display: "inline-block", marginTop: 16, padding: "10px 22px", background: "#111", color: "#fff", borderRadius: 999, textDecoration: "none", fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.3px" }}>
              Trouver un logement
            </Link>
          </div>
        )}
        ...
        {!loading && !error && baux.length > 0 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {baux.map(b => {
              ...
              const depotIntegral = b.depot_restitue_at && Number(b.depot_montant_retenu || 0) === 0
              return (
                <article key={b.id} style={{ background: "white", border: "1px solid #EAE6DF", borderRadius: 20, padding: 24, boxShadow: "0 1px 2px rgba(0,0,0,0.02)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 14, marginBottom: 16 }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, color: "#a16207", background: "#FBF6EA", border: "1px solid #EADFC6", padding: "4px 10px", borderRadius: 999, textTransform: "uppercase", letterSpacing: "1px" }}>
                      Quitté le {formatDateFr(b.bail_termine_at)}
                    </span>
                  </div>

                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 14, marginBottom: 16 }}>
                    <div>
                      <p style={{ fontSize: 10, fontWeight: 700, color: "#8a8477", textTransform: "uppercase", letterSpacing: "1.2px", margin: "0 0 4px" }}>Bailleur</p>
                      <p style={{ fontSize: 13, color: "#111", margin: 0, fontWeight: 500, wordBreak: "break-all" as const }}>{b.proprietaire_email}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      <p style={{ fontSize: 10, fontWeight: 700, color: "#8a8477", textTransform: "uppercase", letterSpacing: "1.2px", margin: "0 0 4px" }}>Loyer mensuel</p>
                      <p style={{ fontSize: 13, color: "#111", margin: 0, fontWeight: 500 }}>
                        {formatEur(b.loyer_hc ? b.loyer_hc + (b.charges || 0) : null)}
                        <span style={{ fontSize: 11, color: "#8a8477", fontWeight: 400 }}> · {formatEur(b.total_loyers_percus)} total</span>
                      </p>
                    </div>
                    <div>
                      ...
                      {b.depot_restitue_at ? (
                        depotIntegral ? (
                          <p style={{ fontSize: 13, color: "#15803d", margin: 0, fontWeight: 500 }}>
                            ✓ {formatEur(b.depot_montant_restitue)} restitué intégralement
                          </p>
                        ) : (
                          <p style={{ fontSize: 13, color: "#111", margin: 0, fontWeight: 500 }}>
                        ...
                        <p style={{ fontSize: 13, color: "#b91c1c", margin: 0, fontWeight: 500 }}>En attente de restitution</p>
                      )}
                    </div>
                  </div>

                  {/* Détail retenues si applicable */}
                  {b.depot_motifs_retenue && b.depot_motifs_retenue.length > 0 && (
                    <details style={{ marginBottom: 14, fontSize: 12, color: "#666" }}>
                      <summary style={{ cursor: "pointer", fontWeight: 600, color: "#a16207" }}>
                        Détail des retenues sur dépôt ({b.depot_motifs_retenue.length})
                      </summary>
                      <ul style={{ margin: "8px 0 0", paddingLeft: 20 }}>
                        {b.depot_motifs_retenue.map((m, i) => (
                          <li key={i} style={{ marginBottom: 4 }}>
                            <strong>{m.libelle}</strong> — {formatEur(m.montant)}
                          </li>
                        ))}
                      </ul>
                    </details>
