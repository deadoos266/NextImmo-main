      ...
      <div style={{ maxWidth: 520, width: "100%", background: "white", borderRadius: 24, padding: isMobile ? "28px 24px" : "40px 44px", boxShadow: "0 4px 32px rgba(0,0,0,0.08)" }}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 24 }}>
          {Array.from({ length: STEPS }).map((_, i) => (
            <div key={i} style={{ flex: 1, height: 4, borderRadius: 999, background: i < step ? "#111" : "#EAE6DF", transition: "background 0.3s" }} />
          ))}
        </div>
        ...
        {step === 1 && (
          <>
            ...
            <p style={{ fontSize: 14, color: "#8a8477", marginBottom: 24, lineHeight: 1.6 }}>
              Cette information nous permet de centrer la carte et pré-remplir vos filtres.
            </p>
            <label style={{ fontSize: 12, fontWeight: 700, color: "#8a8477", textTransform: "uppercase", letterSpacing: "0.5px", display: "block", marginBottom: 6 }}>Ville</label>
            <CityAutocomplete value={ville} onChange={setVille} placeholder="Paris, Lyon, Marseille…" />
            <label style={{ fontSize: 12, fontWeight: 700, color: "#8a8477", textTransform: "uppercase", letterSpacing: "0.5px", display: "block", marginBottom: 6, marginTop: 18 }}>Budget maximum par mois (€)</label>
            <input type="number" value={budgetMax} onChange={e => setBudgetMax(e.target.value)} placeholder="1200"
              style={{ width: "100%", padding: "11px 14px", border: "1px solid #EAE6DF", borderRadius: 10, fontSize: 14, outline: "none", boxSizing: "border-box", fontFamily: "inherit" }} />
          </>
        )}
        ...
        {step === 2 && (
          <>
            ...
            <label style={{ fontSize: 12, fontWeight: 700, color: "#8a8477", textTransform: "uppercase", letterSpacing: "0.5px", display: "block", marginBottom: 8 }}>Nombre de pièces minimum</label>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 18 }}>
              {["1", "2", "3", "4", "5+"].map(p => (
                <button key={p} onClick={() => setPieces(p)}
                  style={{ padding: "10px 18px", borderRadius: 999, border: `1px solid ${pieces === p ? "#111" : "#EAE6DF"}`, background: pieces === p ? "#111" : "white", color: pieces === p ? "white" : "#111", cursor: "pointer", fontWeight: 700, fontSize: 14, fontFamily: "inherit" }}>
                  {p} {p === "1" ? "pièce" : "pièces"}
                </button>
              ))}
            </div>
            <label style={{ fontSize: 12, fontWeight: 700, color: "#8a8477", textTransform: "uppercase", letterSpacing: "0.5px", display: "block", marginBottom: 6 }}>Surface minimum (m²)</label>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[
                ...
                <button key={opt.label} onClick={() => opt.set(!opt.val)}
                  style={{ padding: "14px 18px", border: `1px solid ${opt.val ? "#111" : "#EAE6DF"}`, background: opt.val ? "#111" : "white", color: opt.val ? "white" : "#111", borderRadius: 12, cursor: "pointer", fontWeight: 700, fontSize: 14, fontFamily: "inherit", textAlign: "left", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  {opt.label}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 28, gap: 12 }}>
          {step > 1 ? (
            <button onClick={prev} style={{ background: "none", border: "none", color: "#8a8477", fontWeight: 600, fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>
              ← Retour
            </button>
          ) : <span />}
          <button onClick={next} disabled={saving}
            style={{ background: saving ? "#8a8477" : "#111", color: "white", border: "none", borderRadius: 999, padding: "12px 32px", fontWeight: 700, fontSize: 15, cursor: saving ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 18, marginTop: 20, paddingTop: 16, borderTop: "1px solid #F7F4EF", flexWrap: "wrap" }}>
          <a href="/annonces" style={{ color: "#8a8477", fontSize: 12, fontWeight: 600, textDecoration: "none", letterSpacing: "0.2px" }}>
            Ignorer cette étape
          </a>
          <span style={{ color: "#EAE6DF", fontSize: 12 }}>·</span>
          <a href="/dossier" style={{ color: "#111", fontSize: 12, fontWeight: 700, textDecoration: "none", letterSpacing: "0.2px", borderBottom: "1px solid #111" }}>
