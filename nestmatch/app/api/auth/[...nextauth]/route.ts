...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    async signIn({ user, account }) {
      // For Google sign-ins: upsert user into the users table
      if (account?.provider === "google" && user.email) {
        ...
        if (!existing) {
          ...
          // Keep user object enriched for jwt callback
          user.id = existing.id
    ...
    async jwt({ token, user, account }) {
      if (user) {
        // First sign-in: persist data into the token
        token.id = user.id
        token.role = user.role ?? "locataire"
        token.isAdmin = user.isAdmin ?? false
      }

      // For Google OAuth without an existing user record, fetch freshly
